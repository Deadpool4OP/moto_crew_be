 ...
 * @returns {Promise<User>}
 */
const loginUserWithMobileAndPassword = async (mobile: string, password: string) => {
 ...
 * @returns {string}
 */
const generateToken = (userId: string, expires: string | number, secret = config.jwt.secret) => {
  const payload = {
    sub: userId,
    iat: Math.floor(Date.now() / 1000),
    // exp is handled by sign if we pass expires as a string like '30d'
  };
  return jwt.sign(payload, secret as string, { expiresIn: expires } as jwt.SignOptions);
};
